...
function CardProfile({ formData }: { formData: formData }) {
  ...
  const [openType, setOpenType] = useState<"profile" | "banner" | "about" | null>(null);
    ...
    <motion.div
      ...
      <div className="w-full p-4 transform duration-200">
        <div className="mt-12 grid grid-cols-2">
          <div className="">
            <h1 className="text-xl font-bold text-white">{formData.name || "name"}</h1>
            <p className="font-semibold text-sm text-gray-400">
              @{formData.username || "username"} • {formData.gender || "ไม่ระบุ"}
            </p>
          </div>
        ...
        <div className="mt-4 p-2 border-2 border-gray-400/50 rounded-lg">
          ...
          {formData.about && formData.about.length > 50 ? (
            <p className="text-md text-white whitespace-pre-line">
              ...
              <span className="text-gray-400 ml-1 cursor-pointer duration-200 hover:opacity-80" onClick={() => handleOpenModal("about")}>อ่านเพิ่มเติม</span>
            </p>
          ) : (
            <p className="text-md text-white whitespace-pre-line">
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-2 border-2 border-gray-400/50 rounded-lg">
            <label className="text-gray-400 font-semibold">เกิดมาบนโลกในวันที่</label>
            <p className="text-lg text-white whitespace-pre-line">{formData.birthdate ? `${formatDate(formData.birthdate)}` : "ไม่บอกหรอก"}</p>
          </div>
          <div className="p-2 border-2 border-gray-400/50 rounded-lg">
            <label className="text-gray-400 font-semibold">อายุ</label>
            <p className="text-lg text-white whitespace-pre-line">{formData.birthdate ? `${calculateAge(formData.birthdate)} ปี` : "ไม่บอกหรอก"}</p>
          </div>
        </div>
        <div className="mt-4">
          <Swiper
              ...
              <SwiperSlide key={index}>
                <motion.div
                  onClick={() => {
                    if (activity.name === "Spotify") {
                      window.open(`https://open.spotify.com/track/${activities.data.spotify.track_id}`, '_blank');
                    }
                  }}
                ...
                >
                  <label className="text-gray-400 font-semibold">
                    {activity.name === "Spotify" ? `กำลังฟังเพลง • ${activity.name}` : `กำลังเล่น • ${activity.name}`}
                  </label>
                  <div className="flex gap-4">
                    <div className="w-[90px] h-[90px] rounded-lg overflow-hidden">
                      <img
                        src={activity.name === "Spotify" ? activities.data.spotify.album_art_url : findAppImage(activity)}
                        alt="Song"
                    ...
                    <div className="flex flex-col justify-center">
                      <p className="text-lg text-white whitespace-pre-line font-bold">
                        {activity.name === "Spotify" ? activities.data.spotify.song : activity.name}
                      </p>
                      <p className="text-sm text-gray-400">
                        {activity.name === "Spotify" ? activities.data.spotify.artist : activity.details}
                      </p>
                    </div>
                  </div>
                </motion.div>
      ...
      <Modal centered open={open} onCancel={handleCloseModal} footer={null}>
        {
          openType === "profile" ? (
            ...
            <div className="flex flex-col items-center justify-center text-lg">
              <p className="text-md text-white whitespace-pre-line">{formData.about || "อยากนอนอ้วน"}</p>
            </div>
          )
        }
      </Modal>
