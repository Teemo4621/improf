...
function Edit() {
  ...
  const [previewProfileImageUrl, setPreviewProfileImageUrl] = useState<
    string | null
  >(null);
  const [previewBannerImageUrl, setPreviewBannerImageUrl] = useState<
    string | null
  >(null);
    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-2 w-ful">
        <div className="p-4 md:p-18 ">
          <div className="mb-10">
            <h1 className="text-4xl font-bold text-green-400 flex gap-2">
              สร้างโปรไฟล์ใหม่
            </h1>
            <p className="text-gray-500">
              กรอกแบบฟอร์มด้านล่างนี้เพื่อสร้างโปรไฟล์ใหม่ ✨
            </p>
          </div>
          <Form>
            ...
            <Form.Item style={{ marginBottom: "3.5rem", marginTop: "1rem" }}>
              <div className="relative w-full h-[9rem]">
                <input
                  ...
                  onChange={(e) =>
                    handleImageChange(e, setPreviewBannerImageUrl, "banner")
                  }
                  className="absolute inset-0 opacity-0 cursor-pointer"
                />
                {previewBannerImageUrl ? (
                  <img
                    src={previewBannerImageUrl}
                    alt="Banner"
                    className="w-full h-full object-cover p-1 border-2 border-dashed border-green-400 rounded-lg"
                  />
                ) : formData.banner_image_url ? (
                  <img
                    src={`${(import.meta.env.VITE_API_URL as string).replace("/api/v1", "")}${formData.banner_image_url as string}`}
                    alt="Banner"
                    className="w-full h-full object-cover p-1 border-2 border-dashed border-green-400 rounded-lg"
                  />
                ) : (
                  <div
                    className={`w-full h-full flex items-center justify-center border-2 border-dashed border-green-400 rounded-lg`}
                  >
                ...
                <div className="absolute bottom-[-60px] left-[5rem] transform -translate-x-1/2 w-30 h-30">
                  <input
                    ...
                    accept="image/jpeg, image/png, image/jpg, image/gif"
                    onChange={(e) =>
                      handleImageChange(e, setPreviewProfileImageUrl, "profile")
                    }
                  ...
                  {previewProfileImageUrl ? (
                    <img
                      ...
                      alt="Profile"
                      className="w-full h-full object-cover bg-[#06080E] p-1 border-2 border-dashed border-green-400 rounded-full"
                    />
                  ) : formData.profile_image_url ? (
                    <img
                      src={`${(import.meta.env.VITE_API_URL as string).replace("/api/v1", "")}${formData.profile_image_url as string}`}
                      alt="Banner"
                      className="w-full h-full object-cover bg-[#06080E] p-1 border-2 border-dashed border-green-400 rounded-full"
                    />
                  ) : (
            ...
            <Form.Item style={{ marginBottom: "20px" }}>
              ...
              <Radio.Group
                name="gender"
                onChange={handleInputRadioChangeGender}
                value={formData.gender}
                style={{ backgroundColor: "transparent", color: "white" }}
                buttonStyle="solid"
                className="create-radio-group"
                options={options}
                optionType="button"
              />
            </Form.Item>
            ...
            <Form.Item style={{ marginBottom: "3rem" }}>
              ...
              <DatePicker
                ...
                value={
                  formData.birthdate
                    ? dayjs(formData.birthdate as string)
                    : null
                }
                ...
                className="create-input"
                type="date"
                placeholder="เป็นประชากรบนโลกตั้งแต่"
                needConfirm
              />
            </Form.Item>
            <label className="text-xl text-white font-bold">
            ...
            <Form.Item>
              ...
              <Radio.Group
                name="theme"
                onChange={handleInputRadioChangeTheme}
            ...
            <Form.Item style={{ marginBottom: "10px", marginTop: "20px" }}>
              <button
                ...
                className="w-full bg-green-400 hover:opacity-85 duration-150 text-white cursor-pointer font-bold py-2 px-4 rounded"
              >
                สร้างเลย
              </button>
            </Form.Item>
        ...
        <div className="p-4 md:p-18">
          <div className="mb-20">
            ...
            <p className="text-gray-500">
              ใช่เพื่อเเสดงตัวอย่างโปรไฟล์จากข้อมูลที่ใส่ 🚀
            </p>
          </div>
